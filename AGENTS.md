...
Valid types are `feat`, `fix`, `docs`, `chore`, `refactor`, and `test`. Scopes are optional; use the affected package or area when helpful, e.g. `core`, `nexus`, `tui`, `app`, `desktop`, `sdk`, or `plugin`.
...
- If a namespace-style value is needed, import the module's own exported namespace by name, for example `import { Project } from "@nexus-ai/core/project"`, then reference `Project.ID`.
...
- Tests cannot run from repo root (guard: `do-not-run-tests-from-root`); run from package dirs like `packages/nexus`.

## Type Checking

- Always run `bun typecheck` from package directories (e.g., `packages/nexus`), never `tsc` directly.
